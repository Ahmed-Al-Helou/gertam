...
import SuccessToast from "@/app/(admin)/ui/SuccessToast";
import { useProductById } from "@/hooks/Product/useProductById";
import styles from "@/app/(admin)/edit-product/editProduct.module.css";
...
const InputField = React.memo(({ label, type = "text", value, onChange, placeholder = "", helper = "", required = false }: InputFieldProps) => (
    <div className="mb-4">
        ...
        <input
            ...
            required={required}
            className="w-full border border-gray-300 rounded-xl px-4 py-2 focus:ring-2 focus:ring-blue-400 focus:outline-none shadow-sm"
        />
...
const Step1 = React.memo(({
    ...
    return (
        <div className="bg-blue-50 p-6 rounded-2xl shadow-inner border border-blue-200">
            <InputField label="اسم المنتج (عربي)" value={nameAr} onChange={e => setNameAr(e.target.value)} placeholder="مثال: هاتف ذكي" />
            <InputField label="اسم المنتج (إنجليزي)" value={nameEn} onChange={e => setNameEn(e.target.value)} placeholder="Example: Smart Phone" />
            <TextAreaField label="الوصف (عربي)" value={descriptionAr} onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) => setDescriptionAr(e.target.value)} placeholder="أدخل وصف المنتج بالعربية" />
            <TextAreaField label="الوصف (إنجليزي)" value={descriptionEn} onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) => setDescriptionEn(e.target.value)} placeholder="Enter product description in English" />
            <div className="grid grid-cols-2 gap-4">
                <InputField label="السعر" type="number" value={price} onChange={e => setPrice(e.target.value)} />
                <InputField label="الخصم (%)" type="number" value={discount} onChange={e => setDiscount(e.target.value)} />
            </div>
            ...
            {compatibilities.map((comp, idx) => {
                ...
                return (
                    <div key={idx} className="border p-4 rounded-lg relative">
                        ...
                        <div className="mb-4">
            <label className="block text-gray-700 font-medium mb-1 pt-5">الشركة المصنعة</label>
                            <Select
                        ...
                        <div className="mb-4">
            <label className="block text-gray-700 font-medium mb-1">الموديل</label>
                            <Select
                        ...
                        <div className="grid grid-cols-2 gap-4 mb-4">
                            <div>
                                <label className="block text-gray-700 font-medium mb-1">سنة من</label>
                                <input
                                    type="number"
                                    placeholder="سنة من"
                                    value={comp.yearFrom ?? ""}
                                    onChange={(e) => handleYearChange(idx, Number(e.target.value) || null, comp.yearTo)}
                                    className="w-full border border-gray-300 rounded-xl px-4 py-2 focus:ring-2 focus:ring-purple-400 focus:outline-none shadow-sm"
                                />
                            </div>
                            <div>
                                <label className="block text-gray-700 font-medium mb-1">سنة إلى</label>
                                <input
                                    type="number"
                                    placeholder="سنة إلى"
                                    value={comp.yearTo ?? ""}
...
export default function EditProduct() {
    ...
    useEffect(() => {
        ...
        if (ProductByIdData) {
            ...
            // Convert stored USD to SAR for editing
            const exchangeRate = 3.75; // SAR per USD
            const baseUsd = ProductByIdData.old_price ?? ProductByIdData.price;
            const baseSar = baseUsd ? Number(baseUsd) * exchangeRate : 0;
            setPrice(baseSar ? String(Math.trunc(baseSar)) : "");
...
const percentageChange = Math.abs((newPrice - oldPrice) / oldPrice * 100);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const res = await fetch(`${process.env.NEXT_PUBLIC_SERVER_APP_BASE_URL}/edit-product/${id}`, {
                method: "POST",
                headers: {
                ...
                <div className="rounded-3xl w-full max-w-4xl p-10">
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                        ...
                        <div className="flex justify-between mt-6">
                            {step > 1 && <button type="button" onClick={prevStep} className="px-6 py-3 ml-5 bg-gray-300 text-gray-700 rounded-2xl hover:bg-gray-400 shadow transition">رجوع</button>}
                            {step < 3 && <button type="button" onClick={nextStep} className="ml-auto px-6 py-3 bg-blue-600 text-white rounded-2xl hover:bg-blue-700 shadow transition">التالي</button>}
                            {step === 3 && <button type="submit" className="ml-auto px-6 py-3 bg-green-600 text-white rounded-2xl hover:bg-green-700 shadow transition">حفظ</button>}
                        </div>
                    </form>
                </div>
            </div>
