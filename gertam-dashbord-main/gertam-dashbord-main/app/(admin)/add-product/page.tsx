    ...
    return (
        <div className="bg-blue-50 p-6 rounded-2xl shadow-inner border border-blue-200">
            ...
            <div className="grid grid-cols-2 gap-4">
                <InputField label="السعر بل ريال السعودي" type="number" value={price} onChange={e => setPrice(e.target.value)} />
...
interface Step3Props {
    ...
    setSupportedPaymentMethods: React.Dispatch<React.SetStateAction<string[]>>;
}

const Step3 = React.memo(({ quantity, setQuantity, salesUnit, setSalesUnit, units, supportedPaymentMethods, setSupportedPaymentMethods }: Step3Props) => {
    const handlePaymentMethodToggle = (method: string) => {
        setSupportedPaymentMethods(prev => 
    ...
    return (
        <div className="bg-green-50 p-6 rounded-2xl shadow-inner border border-green-200 space-y-6">
            <div>
            ...
            <div className="border-t pt-6">
                <label className="block text-gray-700 font-semibold mb-4">طرق الدفع المدعومة</label>
                <div className="space-y-3">
                    <label className="flex items-center gap-3 p-3 border rounded-lg cursor-pointer bg-white hover:bg-gray-50 transition">
                        <input
                            type="checkbox"
                            checked={supportedPaymentMethods.includes("cod")}
                            onChange={() => handlePaymentMethodToggle("cod")}
                            className="w-5 h-5"
                        />
                        <span className="text-gray-700">الدفع عند الاستلام (COD)</span>
                    </label>
                    <label className="flex items-center gap-3 p-3 border rounded-lg cursor-pointer bg-white hover:bg-gray-50 transition">
                        <input
                            type="checkbox"
                            checked={supportedPaymentMethods.includes("directpay")}
                            onChange={() => handlePaymentMethodToggle("directpay")}
                            className="w-5 h-5"
                        />
                        <span className="text-gray-700">الدفع الإلكتروني (DirectPay)</span>
                    </label>
                </div>
                {supportedPaymentMethods.length === 0 && (
                    <p className="text-red-600 text-sm mt-2">⚠️ يجب اختيار طريقة دفع واحدة على الأقل</p>
                )}
            </div>
        </div>
    );
...
const [compatibilities, setCompatibilities] = useState<Compatibility[]>([
    ...
    const [supportedPaymentMethods, setSupportedPaymentMethods] = useState<string[]>(["cod"]);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        // إضافة طرق الدفع المدعومة
        supportedPaymentMethods.forEach(method => {
            formData.append("supported_payment_methods[]", method);
        });
        ...
        try {
            ...
            const res = await fetch(`${process.env.NEXT_PUBLIC_SERVER_APP_BASE_URL}/add-product`, {
                method: "POST",
    ...
    setCompatibilities
                        ...
                        {step === 3 && <Step3 {...{quantity, setQuantity, salesUnit, setSalesUnit, units, supportedPaymentMethods, setSupportedPaymentMethods}} />}
